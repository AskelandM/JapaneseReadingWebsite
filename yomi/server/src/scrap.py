            ...
            () => {
                ...
                return Array.from(rows).map(row => {
                    ...
                    return {
                        ...
                        romaji: cells.length === 4 ? cells[2].innerText.trim() : null,
                        english: cells.length === 4 ? cells[3].innerText.trim() : cells[2].innerText.trim()
                    };
...
def insert_words_into_supabase(words, lesson):
    """Stores scraped words into Supabase."""
    lesson_num = lesson.split("lesson=")[-1]
    print(f"✨ Lesson number detected: {lesson_num}")
    for word in words:
...
for lesson in lesson_links:
    ...
    insert_words_into_supabase(words,lesson) 
    print(lesson)
